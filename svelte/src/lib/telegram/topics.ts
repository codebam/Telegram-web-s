...
import {getPeer, getSelfId, peerTitle, previewOf, registerDoc, type StickerItem} from './chats';
...
export async function loadTopicIcon(docId: string): Promise<StickerItem | null> {
  ...
  try {
    ...
    return doc ? registerDoc(doc) : null;
  } catch(err) {
    return null;
  }
}
...
export async function loadTopicIconChoices(): Promise<StickerItem[]> {
  ...
  try {
    ...
    return (set?.documents ?? []).map((doc: any) => registerDoc(doc));
  } catch(err) {
    return [];
  }
}
