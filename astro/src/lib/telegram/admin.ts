...
export async function makeChatPublic(peerId: number, username: string): Promise<number> {
  const {managers} = await bootTelegram();
  // `ChatId` rather than `number`: migrating a basic group to a supergroup returns
  // the new (possibly string) long id — the reason this function returns a peer id
  // at all. The line below already coerces it for the peer id.
  let chatId: ChatId = chatIdOf(peerId);
